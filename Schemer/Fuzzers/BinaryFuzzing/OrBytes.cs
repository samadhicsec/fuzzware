...
{
    class OrBytes : BinaryFuzzerBase
    {
        public OrBytes(ConfigData oConfigData, PreCompData oPreCompData, ObjectDBEntry oSchemaElement)
        ...
        protected override void AssignValueFuzzers(ByteValueFuzzersType FuzzersType)
        {
            ...
            {
                Add("OrBytes", OrBytesFn);
                FuzzingValues = GetByteGroup(FuzzersType.OrBytes.ValueGroupRef);
                BytesVR = GetValueRange(FuzzersType.OrBytes.ValueRangeRef, ValueConfig.ByteRange);
            }
        }

        /// <summary>
        /// Binary Or bytes in the current byte value with values from the user configured list.  If there is an
        /// associated Value Range then only bytes in the specified position will be changed.  Otherwise all the
        /// bytes will be Or'ed, starting with the first and incrementing by one.
        /// </summary>
        public bool OrBytesFn(int FuzzIndex, int NodeIndex)
        {
            return EditBytes(FuzzIndex, NodeIndex, FuzzingValues, BytesVR);
        }

        /// <summary>
        /// Binary Or the current byte array  value with the byte array value from the user configured list
        /// </summary>
